...
function fetchUser() {
    const response = fetch("https://api.github.com/users/TheAlgorithms");
    return response;
}
...
function displayUser(data) {
    ...
    bio.innerHTML = `<h4>${data.bio}</h4>`;

    let location = document.querySelector('#location');
    let span1 = document.createElement('span');
    let span2 = document.createElement('span');
    span1.className = "fa-solid fa-location-dot";
    location.appendChild(span1);
    span1.style.marginRight = "5px";
    span2.innerHTML = `<h4>${data.location}</h4>`;

    span2.style.display = "inline-block";
    location.appendChild(span2);
    

    let twitter = document.querySelector('#links');
    twitter.innerHTML = `<h4> Twitter: ${data.twitter_username}</h4>`;
